...
def extract_activations(data_loader, model, sae_one, sae_two, device):
    ...
    with torch.no_grad():
        for images, labels in tqdm(data_loader, desc="Extracting Activations"):
            ...
            _, hidden_one_act, hidden_two_act, _ = model(images)
...
def evaluate_and_gather_activations(model, sae_one, sae_two, sae_three, data_loader, device):
    """
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Evaluating and Gathering Activations", leave=False):
            ...
            classification_out, hidden_one_act, hidden_two_act, hidden_three_act = model(images)
            
            # --- SAE Forward Pass ---
            reconstructed_one, encoded_one = sae_one(hidden_one_act)
            reconstructed_two, encoded_two = sae_two(hidden_two_act)
            reconstructed_three, encoded_three = sae_three(hidden_three_act)
            ...
            all_encoded_three.append(encoded_three.cpu())
            ...
            recon_errors_three.append(torch.mean((hidden_three_act - reconstructed_three) ** 2).item())
    ...
    avg_recon_error_three = np.mean(recon_errors_three)
    ...
    Z_three = torch.cat(all_encoded_three, dim=0).numpy()
    ...
    results = {
        ...
        "recon_error_three": avg_recon_error_three,
        "Z_one": Z_one,
        "Z_two": Z_two,
        "Z_three": Z_three,
        "y": y
